...
- `tts --stream`: when no `--model` is given, the CLI now auto-selects `sona_speech_1` (the only model that supports streaming), so `supertone tts ... --stream` works without an explicit `-m sona_speech_1`. An explicit incompatible model still errors with exit code 3, and `config default_model` is bypassed on the streaming path. (ISSUE-031, PR #60)
...
- `tests/integration/test_smoke.py` used a non-existent `--json` flag on `voices list`; corrected to `--format json` so `pytest -m integration` runs cleanly when `SUPERTONE_API_KEY` is set.
